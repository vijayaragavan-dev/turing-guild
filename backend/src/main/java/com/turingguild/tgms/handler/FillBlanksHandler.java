...
@Component
public class FillBlanksHandler implements QuestionHandler {

    @Override
    public void evaluate(Answer answer, Question question) {
        if (question.getExpectedOutput() != null) {
            String expected = question.getExpectedOutput().trim().toLowerCase();
            String userAnswer = answer.getAnswerText() != null ? answer.getAnswerText().trim().toLowerCase() : "";
    ...
    public boolean canHandle(Question question) {
        return question.getQuestionType() == QuestionType.FILL_BLANKS;
    }
}
